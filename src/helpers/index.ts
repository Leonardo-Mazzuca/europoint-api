...
const createAchieviments = async (user_id: number) => {
  const userAlreadyHavedAchieviments = await db.user.findFirst({
    where: { id: user_id, achieviments: { some: {} } },
  });
...
export { decodeToken, verifyErrors, createAchieviments, createQuizzes };
